...
import { FileText, Plus, X, GripVertical, Save } from 'lucide-react';
...
export function EditarModeloModal({ open, onOpenChange, modelo }: EditarModeloModalProps) {
  ...
  useEffect(() => {
    if (modelo) {
      ...
      // Carregar campos existentes (exemplo)
      setCampos([
        ...
        { id: '4', tipo: 'textarea', label: 'Exame Físico', obrigatorio: true }
      ]);
    }
  }, [modelo]);
  ...
  const handleAdicionarCampo = () => {
    ...
    if ((novoCampo.tipo === 'select' || novoCampo.tipo === 'radio') && !novoCampo.opcoes.trim()) {
    ...
    const campo: CampoModelo = {
      ...
      opcoes: (novoCampo.tipo === 'select' || novoCampo.tipo === 'radio') 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (campos.length === 0) {
      toast.error('O modelo deve ter pelo menos um campo');
      return;
    }

    toast.success('Modelo atualizado com sucesso!', {
      description: `${campos.length} campo(s) configurado(s)`
    });
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Editar Modelo de Prontuário
          </DialogTitle>
          <DialogDescription>
            Faça alterações no modelo de prontuário
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            {showAddCampo && (
              <div className="border-2 border-dashed border-[#244738] dark:border-green-600 rounded-lg p-4 space-y-4">
                ...
                {(novoCampo.tipo === 'texto' || novoCampo.tipo === 'textarea' || novoCampo.tipo === 'numero') && (
                  <div className="space-y-2">
                ...
                {(novoCampo.tipo === 'select' || novoCampo.tipo === 'radio') && (
                  <div className="space-y-2">
